...
% format long
tol = 5;
% %p = load_parameters_sens_analy_v2();
% p = load_parameters_sens_analy_v3();
% 
% p.eta_F_M = 5.4*1e-1; 
...
p = Homeostasis_calculations_FM(p);
...
for i = 1:8
   ...
   if isnan(mega_metric_matrix(1,i))==1
       ...
       fill([i-1 i-1 i i],[j-1 j j j-1],col_change(val,:),'EdgeColor','none') 
   end
end
figure(fig3)
hold on
...
set(gca,'Xtick',[9,10],'Xticklabel',{'Duration of tissue under 20%','Peak IFN time'})%%
xtickangle(45)
set(gca,'FontSize',18)
ylim([0 16])
...
for j = 41:41+7
    for i = 1:8
       ...
       if isnan(mega_metric_matrix(j,i))==1
           ...
           val = find(grid_metric_1>=mega_metric_matrix(j,i),1);
           fill([i-1 i-1 i i],[j-1-16 j-16 j-16 j-1-16],col_change(val,:),'EdgeColor','none') 
       end
    end
    for i =9:10
        grid_metric_1 = linspace(min(mega_metric_matrix(:,i)),max(mega_metric_matrix(:,i)),9);
...
for j = 25:40%size(mega_metric_matrix,1)
    for i = 1:8
       ...
       if isnan(mega_metric_matrix(j,i))==1
           ...
           val = find(grid_metric_1>=mega_metric_matrix(j,i),1);
           fill([i-1 i-1 i i],[j-1-24 j-24 j-24 j-1-24],col_change(val,:),'EdgeColor','none') 
       end
    end
   for i = 9:10
        grid_metric_1 = linspace(min(mega_metric_matrix(:,i)),max(mega_metric_matrix(:,i)),9);
...
for j = 49:size(mega_metric_matrix,1)-12
    for i = 1:8
       ...
       if isnan(mega_metric_matrix(j,i))==1
           ...
           val = find(grid_metric_1>=mega_metric_matrix(j,i),1);
           fill([i-1 i-1 i i],[j-1-32 j-32 j-32 j-1-32],col_change(val,:),'EdgeColor','none') 
       end
    end
    for i = 9:10
        grid_metric_1 = linspace(min(mega_metric_matrix(:,i)),max(mega_metric_matrix(:,i)),9);
