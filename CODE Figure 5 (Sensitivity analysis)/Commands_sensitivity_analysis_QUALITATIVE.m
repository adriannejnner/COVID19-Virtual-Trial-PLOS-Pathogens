...
[time10plus,sol10plus] = COVID_IMMUNE_MODEL_COPY(p,tspan);% solve model
...
[time10minus,sol10minus] = COVID_IMMUNE_MODEL_COPY(p,tspan);% solve model

plotting_sens_analysis_qualitative(timeORIG,solORIG,time10plus,sol10plus,time10minus,sol10minus);
...
p.p_M_I = p.p_M_I/(1+factor)*(1-factor); %decrease by 10%
p = Homeostasis_calculations(p);% check model is still at homeostasis
tspan = [0 50];
[timeH, solH] = COVID_IMMUNE_MODEL_disease_free_full_model(p,tspan);
p.C_U_0 = p.C_U_0*1.2;
...
p.k_int_L = p.k_int_L/(1+factor)*(1-factor); %decrease by 10%
p = Homeostasis_calculations(p);% check model is still at homeostasis
tspan = [0 50];
[timeH, solH] = COVID_IMMUNE_MODEL_disease_free_full_model(p,tspan);
p.C_U_0 = p.C_U_0*1.2;
...
p.k_B_F = p.k_B_F/(1+factor)*(1-factor); %decrease by 10%
p = Homeostasis_calculations(p);% check model is still at homeostasis
tspan = [0 50];
[timeH, solH] = COVID_IMMUNE_MODEL_disease_free_full_model(p,tspan);
p.C_U_0 = p.C_U_0*1.2;
