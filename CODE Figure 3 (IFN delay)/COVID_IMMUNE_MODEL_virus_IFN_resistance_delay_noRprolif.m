function [time, sol] = COVID_IMMUNE_MODEL_virus_IFN_resistance_noRprolif(p,tspan)
...
dS = p.lam_S*(1-(S+I+D+R)/p.Smax)*S-p.beta*S*V;%/(1+F_B/p.eps_F_I);%+0.001*R;%/(1+F_B/p.eps_F_I)
dI =p.beta/(1+F_B/p.eps_F_I)*ylag1(2)*ylag1(1)-p.d_I*I;%/(1+F_B/p.eps_F_I)
%dR =p.lam_S*(1-(S+I+D+R)/p.Smax)*R+p.beta*S*V/(1+p.eps_F_I/F_B);%-0.001*R;
dR =p.beta*S*V/(1+p.eps_F_I/F_B);%-0.001*R;
dD = p.d_I*I-p.d_D*D;

%dF_U =p.psi_F_prod+p.p_F_I*I/(I+p.eta_F_I)-p.k_lin_F*F_U-p.k_B_F*((p.T_star+I)*p.A_F-F_B)*F_U+p.k_U_F*F_B;
dF_U =p.psi_F_prod+p.p_F_I*ylag2(3)/(ylag2(3)+p.eta_F_I)-p.k_lin_F*F_U-p.k_B_F*((p.T_star+I)*p.A_F-F_B)*F_U+p.k_U_F*F_B;
